...
const TranscriptList = React.memo(({ transcript, helpText, currentUserSpeech, currentThought, errorMessage, onRetry, onDismiss }: { 
  ...
  return (
    <div className="flex-1 min-h-[300px] max-h-[450px] p-4 overflow-y-auto space-y-4 bg-slate-50 relative">
      {errorMessage && (
        <div className="absolute inset-0 z-10 bg-rose-50/95 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={onDismiss}
              className="px-3.5 py-2 bg-white text-slate-700 border border-slate-200 rounded-lg text-xs font-semibold hover:bg-slate-100 transition-colors shadow-sm"
            >
              Dismiss
            </button>
          </div>
...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({ 
          ...
          if (message.toolCall) {
            for (const fc of message.toolCall.functionCalls) {
              if (fc.name === 'update_dashboard_filters') {
                ...
                onUpdateDashboardRef.current(prev => ({ ...prev, activeTab: fc.args.tab as any }));
  ...
  return (
    <>
      ...
      {!isMinimized && (
        <div className={`fixed bottom-24 md:bottom-32 left-0 right-0 md:left-auto md:right-8 mx-4 md:mx-0 md:w-84 bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden z-40 flex flex-col ${isRtl ? 'text-right' : 'text-left'}`} dir={isRtl ? 'rtl' : 'ltr'}>
