...
async function loadExcel(accessor: FileAccessor, fileSize: string): Promise<LoaderResult> {
  ...
  for (const sheetName of sheetNames) {
    ...
    allRows.push(...(jsonRows as Record<string, unknown>[]));
  }
...
async function loadZip(file: File, fileName: string, fileSize: string): Promise<LoaderResult> {
  ...
  for (const [relativePath, zipEntry] of Object.entries(unzipped.files)) {
    if (zipEntry.dir || relativePath.startsWith('__MACOSX') || relativePath.startsWith('.')) continue;

    sheetNames.push(relativePath);
    ...
    const result = await loadSingleFile({
      ...
      getBase64: () => zipEntry.async('base64'),
    }, '0 KB');
